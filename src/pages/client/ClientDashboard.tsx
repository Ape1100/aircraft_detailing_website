...
import { useClientAircraft, useClientInvoices, useClientMembership, useClientRequests } from "@/lib/supabase-client-hooks";
...
export default function ClientDashboard() {
  ...
  const openRequests = requests.filter((r) => !["completed", "paid", "archived"].includes(r.status));
  const unpaidInvoices = invoices.filter((i) => i.status !== "paid");

  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-display text-2xl font-semibold text-ink">Welcome back, {profile?.name.split(" ")[0] ?? "Pilot"}</h1>
      ...
      <div className="grid gap-5 sm:grid-cols-3">
        ...
        <Card>
          <CardHeader>
            ...
            <CardTitle className="text-xl capitalize">{membership?.tier.replace("_", " ") ?? "No membership"}</CardTitle>
          </CardHeader>
        </Card>
      </div>
            ...
            {openRequests.map((r) => {
              ...
              return (
                <div key={r.id} className="flex items-center justify-between rounded-lg border border-ink/10 px-4 py-3">
                  <div>
                    <p className="text-sm font-medium text-ink">{aircraftItem?.tailNumber ?? "Aircraft"}</p>
